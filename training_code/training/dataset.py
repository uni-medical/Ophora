...
class VideoDataset(Dataset):
    def __init__(
        ...
        self.instance_text_data_root = Path(instance_text_data_root) if instance_text_data_root is not None else None
        ...
        if len(self.video_paths) != len(self.prompt_paths):
            raise ValueError(
                f"Expected length of prompts and videos to be the same but found {len(self.prompt_paths)=} and {len(self.video_paths)=}. Please ensure that the number of caption prompts and videos match in your dataset."
            )
    ...
    def __getitem__(self, index: int) -> Dict[str, Any]:
        ...
        if self.load_tensors:
            image_latents, video_latents, prompt_embeds = self._preprocess_video(self.video_paths[index], self.prompt_paths[index])
    ...
    def _load_dataset_from_local_path(self) -> Tuple[List[str], List[str]]:
        ...
        prompt_path = self.instance_text_data_root.joinpath(self.caption_column)
        ...
        with open(prompt_path, "r", encoding="utf-8") as file:
            ...
            prompt_paths = [self.instance_text_data_root.joinpath(line.strip()) for line in file.readlines() if len(line.strip()) > 0]
        with open(video_path, "r", encoding="utf-8") as file:
            video_paths = [self.instance_data_root.joinpath(line.strip()) for line in file.readlines() if len(line.strip()) > 0]
        ...
        return prompt_paths, video_paths
    ...
    def _preprocess_video(self, video_path: Path, prompt_path: Path) -> Tuple[torch.Tensor, Optional[torch.Tensor]]:
        r"""
        ...
        """
        if self.load_tensors:
            return self._load_preprocessed_latents_and_embeds(video_path, prompt_path)
        else:
    ...
    def _load_preprocessed_latents_and_embeds(self, video_path: Path, prompt_path: Path) -> Tuple[torch.Tensor, torch.Tensor]:
        filename_without_ext = video_path.name.split(".")[0]
        pt_filename = f"{filename_without_ext}.npz"
        ...
        video_latents_path = self.instance_data_root
        embeds_path = self.instance_text_data_root
        ...
        '''
        video_data = np.load(video_latent_filepath, allow_pickle=True)
        latents = torch.from_numpy(video_data['image']).float()

        text_data = np.load(embeds_filepath, allow_pickle=True)
        embeds = torch.from_numpy(text_data['caption_embedding']).float()
...
class VideoDatasetWithResizing(VideoDataset):
    ...
    def _preprocess_video(self, video_path: Path, prompt_path: Path) -> torch.Tensor:
        if self.load_tensors:
            return self._load_preprocessed_latents_and_embeds(video_path, prompt_path)
        else:
            video_reader = decord.VideoReader(uri=path.as_posix())
